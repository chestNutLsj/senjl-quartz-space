...
- 用户的特征向量和对应标签即是这个二维空间中的一个点 $(\mathbf{x}_i,y_{i})$，
...
**所谓 PLA，即是在假设集 $\mathcal{H}$ 中选择出最合适的对 *f* 的估计 *g* 的过程**。
...
**可分性**：如果 PLA 能够停止，当且仅当训练集 $\mathcal{D}$ 能够被某个权重 $\mathbf{w}$ 划分且（在训练集上）不再出错。

**线性可分性**：能够以一条线划分的训练集：
- ![[20-Learning-to-Answer-Y-N-PLA-linear-separability.png]]
- 上图左边是线性可分的，中间和右边不是线性可分的；

**因此，对第 2 个问题，只有训练集满足（线性）可分，才能够使前文的 PLA 停止**。

## Guarantee of PLA

> [!tldr] PLA Fact
> - PLA 的过程就是 **当前轮次的权重** $\mathbf{w}_t$ 越来越接近 **目标** $\mathbf{w}_f$ 的过程，
> - 而训练集 $\mathcal{D}$ 线性可分能够保证，一定存在完美的 $\mathbf{w}_f$ 使得对所有训练集中样本的估计 $y_{n}=\text{sign}(\mathbf{w}_{f}^{T}\mathbf{x}_n)$ 成立，
> - 另外，PLA 关键在于遇错更新，这也导致 $\mathbf{w}_{t}$ 的增长并不会太快，即不会线性地、成倍数的增加。

因此，基于这样的事实可以导出如下的数学关系：$\mathbf{w}_f$ 对所有的 $\mathbf{x}_n$ 的判断都是正确的，即==所有样本点都与划分直线有距离（不在直线上）==：
$$
...
- 但这里 $R^{2}$ 和 $\rho$ 的含义是什么？前者是向量最大长度的平方，这个最大长度又称为半径；后者是目标划分直线的法向量与每个样本点的向量的内积的最小值；
...
对于*非线性可分区域*，我们可以改进 PLA 解决，后续学习将会解决这个问题。现在我们先来处理*噪音点*。
...
- 因此，即使对理想的 *f* ，也有可能犯错，但通常情况下 $y_{n}=f(\mathbf{x}_{n})$ 还是成立的。
- 因此我们在运行中获得的 *g* ，对 $y_{n}=g(\mathbf{x}_n)$ 也总是成立的，那么选中一个 *g* ，使得 $\mathbf{w}_{g}\leftarrow\text{arg}\left(\min\limits_{\mathbf{w}}\sum\limits_{n=1}^{N}[y_{n}\ne\text{sign}(\mathbf{w}^{T}\mathbf{x}_{n})]\right)$ ，即取犯错最少的那个 *g* 作为估计，不是就能满足要求吗？
- 确实，但不幸的是，选择犯错最少的 *g* 的算法目前还是个 *NP* 问题，即不存在多项式（polynomial）时间复杂度内解决的算法，它是 $\mathcal{O}(2^n)$ 复杂度的。
