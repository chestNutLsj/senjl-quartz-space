...
* 首先是从 ELF 文件**生成一个全新的地址空间并直接替换进来**（第 15 行），这将导致原有的地址空间生命周期结束，里面包含的全部物理页帧都会被回收；
...
* 第 13 行我们调用 `take_current_task` 来将当前进程控制块从处理器监控 `PROCESSOR` 中取出而不是得到一份拷贝，这是为了**正确维护进程控制块的引用计数**；
