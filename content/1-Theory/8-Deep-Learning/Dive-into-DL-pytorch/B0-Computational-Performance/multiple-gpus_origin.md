...
![Model parallelism in the original AlexNet design due to limited GPU memory.](alexnet-original.svg)
...
![Parallelization on multiple GPUs. From left to right: original problem, network partitioning, layer-wise partitioning, data parallelism.](splitting.svg)
:label:`fig_splitting`
...
![Calculation of minibatch stochastic gradient descent using data parallelism on two GPUs.](data-parallel.svg)
