...
![左图是单GPU训练；右图是多GPU训练的一个变体：（1）计算损失和梯度，（2）所有梯度聚合在一个GPU上，（3）发生参数更新，并将参数重新广播给所有GPU](ps.svg)
...
![在8台V100 GPU服务器上连接NVLink（图片由英伟达提供）](nvlink.svg)
:label:`fig_nvlink`
...
![将NVLink网络分解为两个环。](nvlink-twoloop.svg)
...
![跨4个节点的环同步。每个节点开始向其左邻居发送部分梯度，直到在其右邻居中找到聚合的梯度](ringsync.svg)
:label:`fig_ringsync`
...
![多机多GPU分布式并行训练](ps-multimachine.svg)
...
![上图：单参数服务器是一个瓶颈，因为它的带宽是有限的；下图：多参数服务器使用聚合带宽存储部分参数](ps-multips.svg)
