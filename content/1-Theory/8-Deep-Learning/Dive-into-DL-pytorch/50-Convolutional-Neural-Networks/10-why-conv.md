...
这看起来类似于 `5.1.3` ，但有一个主要区别：这里不是使用 $(i+a, j+b)$，而是使用 $(i-a, j-b)$ 。当然，这种区别只是表面的，因为我们总是可以匹配 `5.1.3` 和 `5.1.6` 之间的符号。我们在 `5.1.3` 中的原始定义更正确地描述了***互相关***（cross-correlation），这个问题将在下一节中讨论。
